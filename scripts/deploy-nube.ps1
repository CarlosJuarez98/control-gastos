# Deploy código control-gastos a OCI (sin sync de datos).
...
Write-Host "== Deploy control-gastos (código) ==" -ForegroundColor Cyan
Push-Location $Root
...
exit 1
"@
if ($LASTEXITCODE -ne 0) { throw "Health check en contenedor falló" }
...
Write-Host "Aviso: contenedor OK pero no respondió la URL pública (Caddy/DNS)." -ForegroundColor Yellow
